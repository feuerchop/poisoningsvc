...
class View(object):
    ...
    def plot_contour(self, model):
        ...
        if type(model.clf).__name__ is "SVC":
            ...
            c3 = self.ax.contour(Ax,Ay, dists.reshape(Ax.shape), levels=[0], colors='k', linestyles='solid', linewidths=[2])
            if self.toolbar.is_surface.get():
                # plot the error surface
                dataset = {"train": X, "train_labels": y}
                errors = np.asarray([lpn.objective(all_points_x[id,:], model.clf, dataset, target='fval') for id in range(N)])
                c4 = self.ax.contourf(Ax,Ay, errors.reshape(Ax.shape), 100, cmap='jet', alpha=0.5)
                # c5 = self.fig.colorbar(c4, format='%2.3f', aspect=12)
                # self.contours.append(c5)
            else:
                c4 = self.ax.contourf(Ax,Ay, dists.reshape(Ax.shape), 10, cmap='bone', alpha=0.5)
